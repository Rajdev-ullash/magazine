<div class="sidebar flex flex-col gap-12 md:max-w-[380px]">
    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="justify-start items-start gap-2 inline-flex flex-wrap">
            @foreach ($categories as $category)
                <a class='button-7 hover-up' href="{{ url('category/' . $category->id) }}">{{ $category->name }}</a>
            @endforeach
        </div>
    </div>
    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="w-72 h-96 relative rounded-2xl">
            <img class="w-72 h-96 left-0 top-0 absolute rounded-2xl" src="{{ asset('user/imgs/pages/banner1.png') }}" />
            <div
